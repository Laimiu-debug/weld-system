...
class Permission(Enum):
    ...
    IMPORT_MODIFY = "import.modify"
    IMPORT_REVIEW = "import.review"
    IMPORT_PUBLISH = "import.publish"
    IMPORT_KEY_MANAGE = "import.key_manage"
...
class PermissionService:
    ...
    def has_permission(
        self, user_id: int, permission: str, is_admin: bool = False
    ) -> bool:
        """检查用户是否有指定权限"""
        if is_admin:
    ...
    def has_any_permission(
        self, user_id: int, permissions: List[str], is_admin: bool = False
    ) -> bool:
        """检查用户是否有任意一个指定权限"""
        if is_admin:
    ...
    def check_permission(self, user_id: int, permission: str, is_admin: bool = False):
        ...
        if not self.has_permission(user_id, permission, is_admin):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"权限不足，需要权限: {permission}"
            )
        ...
        if user.member_tier:
            permissions.update(
                ...
                if permission.value.startswith(
                    ("engineering.", "import.", "capability.", "rules.")
                )
            )
