...
class WeldingMaterial(Base):
    ...
    workspace_type = Column(
        String(20), nullable=False, default="personal", index=True, comment="工作区类型"
    )
    company_id = Column(
        Integer,
    ...
    density_g_cm3 = Column(Float, comment="密度(g/cm³)")
    default_deposition_efficiency = Column(Float, comment="默认熔敷效率")
    default_deposition_rate_kg_h = Column(Float, comment="默认熔敷速度(kg/h)")
    consumable_type = Column(String(50), comment="定额消耗品类型")
    applicable_welding_methods = Column(
        JSONB, nullable=False, default=list, comment="适用焊接方法"
    )
    ...
    parameter_version = Column(Integer, nullable=False, default=1, comment="定额参数版本")
    parameter_approval_status = Column(
        String(20), nullable=False, default="draft", comment="定额参数审批状态"
    )
    parameter_effective_from = Column(DateTime, comment="定额参数生效时间")
    parameter_effective_to = Column(DateTime, comment="定额参数失效时间")
...
class MaterialCategory(Base):
    ...
    parent_id = Column(
        Integer, ForeignKey("material_categories.id"), nullable=True, comment="父分类ID"
    )
...
class MaterialTransaction(Base):
    ...
    material_id = Column(
        Integer,
        ForeignKey("welding_materials.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
        comment="焊材ID",
    )
    ...
    transaction_type = Column(
        ...
        nullable=False,
        index=True,
        comment="交易类型：in/out/adjust/return/transfer/consume",
    )
    transaction_number = Column(String(100), index=True, comment="交易单号")
    transaction_date = Column(
        DateTime, default=datetime.utcnow, nullable=False, comment="交易日期"
    )
