...
class DocumentType(str, enum.Enum):
    ...
    WELD_SEQUENCE_VERSION = "weld_sequence_version"
...
class ApprovalWorkflowDefinition(Base):
    ...
    # 索引
    __table_args__ = (Index("ix_workflow_company_type", "company_id", "document_type"),)
...
class ApprovalInstance(Base):
    ...
    workflow_id = Column(
        Integer,
        ForeignKey("approval_workflow_definitions.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
        comment="工作流定义ID",
    )
    ...
    # 文档关联
    document_type = Column(String(50), nullable=False, index=True, comment="文档类型")
    document_id = Column(Integer, nullable=True, index=True, comment="旧业务表数字ID")
    document_ref = Column(String(100), nullable=False, comment="稳定对象引用")
    version_key = Column(String(100), nullable=False, comment="审批版本标识")
    version_snapshot = Column(JSONB, nullable=False, default=dict, comment="不可变版本快照")
    snapshot_hash = Column(String(64), nullable=False, comment="版本快照SHA256")
    document_number = Column(String(100), comment="文档编号")
    document_title = Column(String(200), comment="文档标题")

    # 工作区信息
    workspace_type = Column(
        String(20), nullable=False, default="enterprise", index=True, comment="工作区类型"
    )
    company_id = Column(
        Integer,
        ForeignKey("companies.id", ondelete="CASCADE"),
    ...
    # 审批状态
    status = Column(
        String(20), nullable=False, default="draft", index=True, comment="审批状态"
    )
    ...
    # 提交信息
    submitter_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, index=True, comment="提交人ID"
    )
    ...
    workflow_definition = relationship(
        "ApprovalWorkflowDefinition", back_populates="instances"
    )
...
class ApprovalHistory(Base):
    ...
    operator_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, index=True, comment="操作人ID"
    )
...
class ApprovalNotification(Base):
    ...
    notification_type = Column(
        String(50),
        nullable=False,
        comment="通知类型: submitted, approved, rejected, returned, reminder",
    )
