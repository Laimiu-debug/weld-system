...
schema='qa_materials_f05_20260905'
...
with SessionLocal() as db:
 users=[]
 for name,tier in [('f05_personal','personal_flagship'),('f05_enterprise','enterprise'),('f05_outsider','personal_flagship')]:
  user=User(username=name,email=name+'@example.com',hashed_password=get_password_hash(password),full_name='F05 专用测试账号',is_active=True,is_verified=True,member_tier=tier,membership_type='enterprise' if tier=='enterprise' else 'personal',subscription_status='active',subscription_end_date=datetime.utcnow()+timedelta(days=7))
  db.add(user);db.flush();users.append(user)
 company=Company(name='F05 焊材验收测试企业',owner_id=users[1].id,membership_tier='enterprise',is_active=True)
 db.add(company);db.flush()
 factory=Factory(name='F05 测试工厂',code='F05-FACTORY',company_id=company.id)
 ...
 path=Path(os.environ['TEMP'])/'weld-f05-credentials.json'
