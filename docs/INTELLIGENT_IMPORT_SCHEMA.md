...
字段基础、动态 Schema、编辑入口、暂存数据层、私有原件上传、分页解析、视觉 OCR、动态结构化提取和首个智能导入页面已经完成；尚未实现额度、字段审核修改、正式发布与 PQR 正式闭环。
...
当前开放的接口位于 `/api/v1/smart-import`，支持创建/查询批次、登记带 SHA-256 的文档、上传私有原件、触发分页解析、读取页面记录、执行 OCR/模型提取、读取字段证据，以及创建手工草稿。字段审核修改和正式发布仍属于后续切片。
