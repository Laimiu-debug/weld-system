...
字段基础、动态 Schema、编辑入口、暂存数据层、私有原件上传、分页解析、视觉 OCR、动态结构化提取、字段审核、平台额度账本和 WPS/PQR 正式发布闭环已经完成；尚未实现企业保存 Key、完整 PDF 区域联动和异步批处理。
...
当前开放的接口位于 `/api/v1/smart-import`，支持创建/查询批次、登记带 SHA-256 的文档、上传私有原件、触发分页解析、读取页面记录、执行 OCR/模型提取、读取字段证据、字段接受/修正/拒绝、批量接受高置信度字段，以及创建手工草稿。审核完成的 WPS/PQR 可经现有 Service 发布为正式业务草稿，再继续沿用原审批流程。
...
AI 权益由 `ai_plan_entitlements` 数据驱动，统一限制单任务页数、工作区日/月任务数与并发数；企业工作区同时限制成员个人的日/月任务数与并发数。平台额度额外执行每日和每月点数预占，BYOK 不扣平台点数但不能绕过任务及并发边界。

企业智能导入权限按动作拆分为上传、识别、修改、审核、发布和 Key 管理，并预留 `engineering.*`、`capability.*`、`rules.*` 领域权限。审批实例使用稳定对象引用、版本键、完整快照与 SHA256 绑定具体版本；受支持的旧业务文档在每次审批动作前重新计算快照，发现静默修改即拒绝继续审批。

原件上传、下载以及每次 OCR/模型提取都会写安全审计事件。审计只保存用户、工作区、文档 ID/哈希、供应商、模型、阶段和页码，不保存文档正文、私有存储 Key 或 API Key。AI 未配置时，原有手工 WPS、PQR、焊工及手工导入草稿入口保持独立可用。
...
- `extraction_jobs` 保存 Provider、模型、Prompt 版本、内部追踪号、外部响应号和 Token 用量。
