...
- [ ] T01 权限拒绝统一抛出 403，修复忽略 False 与操作名称大小写不一致；补普通员工修改他人数据测试。
- [ ] T02 附件增加用户/企业/业务记录归属，下载校验业务权限；上传采用有界读取并清理失败附件。
- [ ] T03 修复生产计划等模型与工作区权限组件的字段契约；已模拟复现普通企业员工读取计划缺少 access_level 报错。
- [ ] T04 支付状态、订阅、会员权益统一事务；数据库层幂等及失败补偿，通知单独重试。
- [x] T05 通用业务 CRUD 的生产计划、质量标准、员工绩效、报表模板均通过明确模型校验；更新合并现值后重新验证，拒绝额外字段和客户端写入归属、审计及快照字段。详见批次 10。
- [ ] T06 渐进统一 API 响应类型，拆分智能导入等大页面，补业务交互回归。
...
- [ ] AI01 修复图纸识别“服务器内部错误”，核对真实模型请求、结果校验和入库异常。
- [ ] AI02 修复 PQR 视觉识别、OCR 失败后重试和结构化字段生成链路。
- [ ] AI03 修复图纸多阶段识别被前端 / 网关超时截断的问题。
- [ ] AI04 统一图纸模型路由与数据接收方展示，避免按默认模型授权却调用另一个服务。
- [ ] AI05 补齐 CAD 输入；明确 DXF 与 DWG 转换依赖和失败提示，不能将未支持的格式标为可识别。
- [ ] AI06 使用实际视觉模型以及图纸 / PQR 样本验收；记录已验证范围，识别结果仍须人工核对。

以下为上一轮审查发现的后续待办，按批次推进并记录本地验证结果：

- [ ] SQ01 补齐生产放行、派工、资源授权、执行和变更申请的前端操作入口。批次 6～7 已接入放行、任务查询、普通派工、完工登记及资格特批申请/处理；变更流程、领用单关联及真实账号联调仍待完成。
- [x] SQ02 执行工序时校验前置工序及质量条件，避免绕过依赖直接完工。
- [x] SQ03 封闭容器前校验内部焊缝的无损检测完成条件。
- [x] SQ04 执行时重新核对焊工资格和设备有效状态，避免派工后过期仍可执行。
- [ ] SQ05 将分段、跳焊、对称策略落实为实际步骤与排序，并验证拓扑排序不会覆盖策略。
- [ ] SQ06 已放行焊序重算接入已批准变更申请，提供完整操作流程。
- [ ] SQ07 产品结构识别与焊序模板适配，减少依靠名称关键词套用压力容器模板。
